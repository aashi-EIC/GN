...
const block = z.discriminatedUnion('type', [
  ...
  z.object({
    ...
    chart_type: z.enum([
      'line', 'area', 'bar', 'stacked-bar', 'horizontal-bar', 'pie', 'donut',
      'scatter', 'bubble', 'heatmap', 'radar', 'funnel', 'gauge',
    ]),
    title: z.string().min(1).max(500).optional(),
...
const currentMcpResponse = z.object({
  ...
  tool_result: z.array(
    z.array(z.record(z.string(), scalar)).max(10_000),
  ).max(100).default([]),

...
const unsafe = [
  /<\s*script\b/i, /<\s*(?:iframe|object|embed|applet|meta|base)\b/i,
...
export function assertSafeText(value: string, label: string) {
  if (unsafe.some((pattern) => pattern.test(value))) throw new UnsafeContentError(`${label} contains prohibited active content`);
}
...
export function assertSafeUnknown(value: unknown, label = 'MCP response', depth = 0): void {
  ...
  if (Array.isArray(value)) return value.forEach((item) => assertSafeUnknown(item, label, depth + 1));
  if (value && typeof value === 'object') {
    for (const [key, child] of Object.entries(value)) {
      assertSafeText(key, label);
      assertSafeUnknown(child, label, depth + 1);
    }
  }
}
